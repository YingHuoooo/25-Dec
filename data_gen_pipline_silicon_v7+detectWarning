...
# ===========================================
WORKER_TIMEOUT = 8  # 子进程超时时间（秒）
# ===========================================

# Mac/conda 建议使用 spawn，避免 fork 带来的崩溃
try:
...
def ensure_single_solid(local_vars):
    """
    Fuse 所有 solid，确保仅 1 个实体；否则抛出异常。
    避免使用即将废弃的 combineSolids。
    """
...
def worker_generate_sample(out_queue):
    """
    在子进程中生成并验证几何，返回 geo_data 或 None。
    只做几何，不调用模型。
    """
    try:
        gen = ComplexShapeGenerator()  # 在子进程内实例化，避免 pickle 问题
        geo_data = gen.generate_complex_sample()
        local_vars = {}
        ...
        if wp.val().Volume() < 1e-6:
            out_queue.put(None)
            return
        out_queue.put(geo_data)
    except Exception:
        out_queue.put(None)

class ComplexShapeGenerator:
    """
    复杂几何生成器：更多基体 + 多工序 + 多面分布 + 单体安全约束
    """
    ...
    def generate_base(self):
        ...
        if base_type in ["box", "rounded_box"]:
            ...
            dims.update({"a": a, "b": b, "H": H, "thickness": thickness,
                         "footprint_L": 2*a, "footprint_W": 2*b})
            ...
            dims.update({"R": footprint/2, "H": H, "side_len": side_len,
                         "footprint_L": footprint, "footprint_W": footprint})
    ...
    def add_feature(self, code, skel, params, dims):
        base_type = dims["type"]
        features = [
            "top_hole", "counterbore",
            "circular_array_holes", "grid_array_holes",
            "slot", "radial_slot",
            "top_pocket", "pocket_circle",
            "ring_groove",
            "top_boss", "side_boss",
            "side_hole", "side_slot",
            "keyway_slot",
        ...
        if base_type in ["tube", "ellipse_tube"]:
            forbid = {"top_pocket", "slot", "radial_slot", "circular_array_holes",
                      "grid_array_holes", "ring_groove", "top_boss",
                      "side_boss", "side_slot", "side_hole", "keyway_slot"}
        ...
        if choice == "top_hole":
            ...
            signed_d = self.signed_depth(face, depth)
            code += (
                f".faces('{face}').workplane().polarArray({arr_r}, 0, 360, {count}).slot({length}, {width}).cutBlind({signed_d})"
            )
...
def main():
    ...
    print(f"🚀 开始生成 CNC 数据集 (多基体 & 多工序 & 多面 & 单体安全版)")
    ...
    with open(OUTPUT_FILE, "a", encoding="utf-8") as f:
        ...
        while success_count < NUM_SAMPLES:
            ...
            # 带超时读取队列，避免 empty()+get() 卡住
            geo_data = None
            try:
                geo_data = q.get(timeout=0.5)
            except queue.Empty:
            ...
            set_stage(pbar, "geom ok -> LLM")
            description = generate_text_with_qwen(geo_data['code'])
            if not description:
                set_stage(pbar, "LLM failed/timeout")
                continue
