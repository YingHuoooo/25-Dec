...
# 配置 Gemini pip install google-generativeai cadquery tqdm
...
def generate_text_with_gemini(code_str, pbar=None):
    """
    ...
    try:
        # --- 优化点：在等待时通知进度条 ---
        if pbar: pbar.set_description("⏳ API冷却中 (4.5s)...")
        time.sleep(4.5) 
        
        if pbar: pbar.set_description("🚀 正在请求 Gemini...")
        response = model.generate_content(prompt)
        return response.text.strip()
    except Exception as e:
        # 出错时在进度条上方打印错误，不破坏进度条显示
        if pbar: pbar.write(f"⚠️ API Error: {e}")
        time.sleep(10)
        return None
...
def main():
    ...
    success_count = 0
    # 统计失败次数，显示在进度条上
    stats = {"gen_fail": 0, "api_fail": 0}
    
    with open(OUTPUT_FILE, "a", encoding="utf-8") as f:
        # 使用 tqdm，并设置单位
        pbar = tqdm(total=NUM_SAMPLES, unit="sample")
        
        while success_count < NUM_SAMPLES:
            # 1. 生成几何
            pbar.set_description("🎲 生成随机几何...")
            geo_data = generator.generate_complex_sample()
            
            # 2. 再次最终验证
            try:
                local_vars = {}
                # 捕获标准输出以防止 cadquery 产生的额外打印干扰进度条
            ...
            # 3. 调用 Gemini 生成文本 (传入 pbar 用于内部状态更新)
            description = generate_text_with_gemini(geo_data['code'], pbar)
